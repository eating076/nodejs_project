...
app.post('/steps', (req, res) => {
    const { user_id, date, steps, goal } = req.body;
    if (!user_id || !date || !steps || !goal) {
        return res.status(400).send('user_id, date, steps, and goal are required');
    }
    const completionRate = ((steps / goal) * 100).toFixed(2);

    const query = `
        INSERT INTO steps (user_id, date, steps, goal, completion_rate) 
        VALUES (?, ?, ?, ?, ?)
    ...
    connection.query(query, [user_id, date, steps, goal, completionRate], (err, result) => {
        if (err) {
            console.error('Error executing query:', err);
            res.status(500).send(`Error saving data: ${err.message}`);
            return;
...
app.post('/analysis', (req, res) => {
    ...
    const { user_id, test_date, test_score, analysis_advice } = req.body;

    console.log('user_id:', user_id); // 檢查 user_id 值
    console.log('test_date:', test_date); // 檢查 test_date 值
    ...
    const query = `
        INSERT INTO analysis (user_id, test_date, test_score, analysis_advice) 
        VALUES (?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE 
            test_score = VALUES(test_score), 
    ...
    connection.query(query, [user_id, test_date, test_score, analysis_advice], (err, result) => {
        if (err) {
            console.error('Error executing query:', err);
            res.status(500).send(`Error saving data: ${err.message}`);
            return;
...
app.post('/advanced-search', (req, res) => {
    const { user_id, child_order, name, birthdate, blood_type } = req.body;

    if (!user_id || !child_order || !name || !birthdate || !blood_type) {
        return res.status(400).send('All fields are required and cannot be null');
    }

    const query = `
        INSERT INTO advanced_search (user_id, child_order, name, birthdate, blood_type) 
        VALUES (?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE 
    ...
    connection.query(query, [user_id, child_order, name, birthdate, blood_type], (err, result) => {
        if (err) {
            console.error('Error executing query:', err);
            res.status(500).send(`Error saving data: ${err.message}`);
            return;
